...
export default function FichaAnamnese() {
  const { obterAnamnese, editarAnamnese, loading, error } = useAluno();
  ...
  const fetchDados = async () => {
    const response = await obterAnamnese();
    ...
    setNomeResponsavel(data.nomeDoResponsavel);
    setContatoResponsavel(data.contatoDoResponsavel);
    setPeso(String(data.peso));
    setAltura(String(data.altura));

    setExtensao(data.participaDeExtensao);
    setAtvFisica(data.praticaAtividade);
    setSintomas(data.boxSensacoes || []);
    setNutricionista(data.fazDieta);

    setRefeicao(String(data.quantasRefeicoes));
    setHorasNoite(String(data.horasDeSono));
    setAguaDia(String(data.litrosDeAgua));

    setColesterol(data.temColesterolAlto);
    setHdl(data.hdl);
    setLdl(data.ldl);
    setTrigliceridos(data.temTrigliceridesAlto);

    setDiabetes(data.diabetico);
    setTipoDiabetes(data.tipoDeDiabete?.[0] || "");
    setMendicamentoInsulina(data.descDiabete);

    setHipertenso(data.hipertenso);
    setMedicamentoHipertenso(data.descHipertenso);

    setAsma(data.asma);
    setBroncodilatador(data.descAsma);

    setProblemaRespiratorio(data.temProblemasRespiratorios);
    setProblemaRespiratorioDesc(data.descProblemasRespiratorios);

    setPaisObesoso(data.paisObesos);
    setCirurgia(data.jaFezCirurgia);
    setDescCirurgia(data.descCirurgia);

    setTipoMedicamento(data.usaMedicamento);
    setDescMedicamento(data.descMedicamento);

    setRecomendaMedica(data.recomendacaoMedica);
    setDescProbCol(data.descDesconforto);
    setRestricao(data.descRestricaoMedica);

    setMotivo(data.objetivos || []);
    setMotivoTreino(data.descOutro);

    setMedicoDoenca(data.problemaDeCoracao);
    setDoresPeito(data.doresNoPeito);
    setDoresPeitoMes(data.doresNoPeitoEmAtividade);
    setDesequilibrio(data.desequilibrio);
    setProblemaOsseo(data.problemaOsseo);
    setMedicamentoPressao(data.medicamentoParaPressao);
    setRazaoNaoExerc(data.outrasRazoes);
  }
